package exercise_three.classes;

import org.jboss.logging.Logger;
/**
 * Clase MessagesFunction
 * Esta clase permite mostrar los mensajes que son invocados por métodos para mostrar información al usuario
 *
 * Autor
 * John Edward Acevedo Rojas -- jhedacro@gmail.com
 *
 * Versión: v1-29/05/22
 */
public class MessagesFunction {
    /**
     * Se crea una instancia de la clase logger, la cual permite mostrar mensajes en pantalla
     */
    private static final Logger logger = Logger.getLogger(MessagesFunction.class);
    /**
     * Método getIntance
     * Crea una instancia de la clase MessageFunction
     * @return una instancia de la clase
     */
    public static MessagesFunction getInstance(){return new MessagesFunction();}
    /**
     * Método showWelcome
     * Permite mostrar un mensaje de bienvenida
     */
    public void showWelcome(){logger.info("Bienvenido al Ejercicio 3 con POO");}

    /**
     * Método getRadio()
     * Permite obtener el valor del radio mediante un mensaje mostrado al usuario por pantalla para el ingreso de dicho valor
     */
    public void getRadio(){logger.info("Ingrese el valor del radio: ");}

    /**
     * Método getArea
     * permite Mostrar en pantalla el valor del area obtenido
     * @param area valor del area calculado
     */
    public void getArea(double area){logger.info("El valor del area es: : "+area);}
}
